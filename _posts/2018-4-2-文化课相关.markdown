...
_先考虑简单情况 设<a href="http://www.codecogs.com/eqnedit.php?latex=a&plus;b<c&plus;d" target="_blank"><img src="http://latex.codecogs.com/gif.latex?a&plus;b<c&plus;d" title="a+b<c+d" /></a>（a,b,c,d均为化学计量数），且反应物生成物均为气体.因为质量守恒，所以增加压强实际上是减小了体积，要想让压强回去，就是让体积再变大呀，那就是让a+b的一边的反应加快(假设此时温度不变).即<a href="http://www.codecogs.com/eqnedit.php?latex={\color{Green}&space;_V{正}>^{_{V逆}}}" target="_blank"><img src="http://latex.codecogs.com/gif.latex?{\color{Green}&space;_V{正}>^{_{V逆}}}" title="{\color{Green} _V{正}>^{_{V逆}}}" /></a>，就OK了._
